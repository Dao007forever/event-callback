...
describe('Handler', function() {
    ...
    describe('delete', function(done) {
        ...
        beforeEach(function() {
            options1 = { url: 'http://example.com' };
            options2 = { url: 'http://example2.com' };
        });

        var setUp = function(options1, options2) {
            return Q.all([
                pclient.sadd('event:test', 'action'),
                pclient.sadd('action:test:action', '1'),
        ...
        it('should remove matching actions if an option is given', function(done) {
            var json = { event: 'test', action: 'action', options: options2 };
            setUp(options1, options2)
                .then(function() {
                    return handler.delete(json)
                        .then(function() {
                            return Q.all([
                                ...
                                .spread(function(actions, actionExists, option1, option2) {
                                    actions.should.eql([ 'action' ]);
                                    actionExists.should.equal(1);
                                    option1.should.equal(1);
                                    option2.should.equal(0);
                                    done();
                                });
                        });
